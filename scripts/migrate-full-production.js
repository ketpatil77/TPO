...
async function fetchD1Count(tableName) {
    const output = execSync(`npx wrangler d1 execute ait-tpo-production --remote --command="SELECT count(*) as count FROM ${tableName};"`, {
...
async function migrateTable(tableName) {
    ...
    for (let b = 0; b < totalBatches; b++) {
        ...
        try {
            execWithRetry(`npx wrangler d1 execute ait-tpo-production --file="${tempSqlFile}" --remote`, {
                env: { ...process.env, CLOUDFLARE_API_TOKEN },
                stdio: 'pipe'
            });

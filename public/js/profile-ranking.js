(() => {
        ...
        panel.innerHTML = `
            <div class="ranking-hero glass-card ranking-trust-hero">
                <div>
                    ...
                    <p>Every point comes from a published rule. Evidence must be verified before it can change a rank.</p>
                    <div class="ranking-trust-strip">
                        <span>Automatic calculation</span><span>No manual rank editing</span><span id="rankingRuleVersion">Rules —</span>
                    </div>
                </div>
            ...
            <div id="rankingList" class="ranking-list" aria-live="polite"><div class="panel-empty">Loading verified Profile Points…</div></div>
    ...
    function installOverviewSpotlight() {
        ...
        card.innerHTML = `
            <div class="overview-rank-copy">
                ...
                <p id="overviewRankSubline">Only verified evidence affects Profile Points.</p>
                <div id="overviewRankTrust" class="overview-rank-trust"></div>
            </div>
            <div class="overview-rank-score"><span>Profile Points</span><strong id="overviewRankPoints">—</strong><small id="overviewRankEvidence">Verified evidence</small></div>
    ...
    async function loadOverviewSpotlight() {
        ...
        try {
            ...
            document.getElementById('overviewRankEvidence').textContent = `${me.evidence_counts?.verified || 0} verified · ${me.evidence_counts?.pending || 0} pending`;
    ...
    async function loadRanking() {
        ...
        if (!list) return;
        list.innerHTML = '<div class="panel-empty">Calculating verified Profile Points…</div>';
    ...
    function evidenceItem(item) {
        ...
        return `<div class="ranking-evidence-item">
            <div><strong>${escapeHtml(item.label)}</strong><small>${escapeHtml(item.reason || '')}</small>${links ? `<div class="ranking-evidence-links">${links}</div>` : ''}</div>
    ...
    function render(data) {
        ...
        summary.innerHTML = current
            ? `<span>Your verified position</span><strong>#${current.rank}</strong><small>${current.points} points · ${current.evidence_counts?.verified || 0} verified · ${current.evidence_counts?.pending || 0} pending</small>`
        ...
        document.getElementById('rankingPodium').innerHTML = top.map((row, index) => `<article class="glass-card podium-card podium-${index + 1}">
            ...
            <small>${row.evidence_counts?.verified || 0} verified evidence records</small>
        </article>`).join('');
        ...
        document.getElementById('rankingList').innerHTML = data.rows.length ? data.rows.map(row => `
            <article class="glass-card ranking-row ${row.is_me ? 'ranking-row-me' : ''}">
                ...
                <div class="ranking-person"><strong>${escapeHtml(row.name)}${row.is_me ? ' · You' : ''}</strong><span>${escapeHtml(row.branch)} · ${escapeHtml(row.year || '—')}</span><small>${row.evidence_counts?.verified || 0} verified · ${row.evidence_counts?.pending || 0} pending</small></div>
                <div class="ranking-points"><strong>${row.points}</strong><span>verified points</span></div>

(() => {
    ...
    function loadStyles() {
        ...
        link.rel = 'stylesheet';
        link.href = '/css/certificate-vault.css?v=20260903-1';
    ...
    function ensureUploadField() {
        ...
        block.innerHTML = `
            ...
            <div class="form-hint">JPG, JPEG or PNG only. No PDFs. The image is compressed on your device before upload.</div>
    ...
    async function vaultSubmit(event) {
        ...
        try {
            ...
            if (hadPreparedImage) {
                ...
                const evidenceResponse = await fetch(`/api/student/certificate-evidence/${certificateId}`, { method: 'POST', body: uploadForm });
            ...
            if (typeof showToast === 'function') showToast(hadPreparedImage ? 'Certificate and proof saved.' : (existing?.evidence_path ? 'Certificate updated. Existing proof kept.' : 'Certificate saved.'), 'success');

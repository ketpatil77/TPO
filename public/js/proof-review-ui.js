(() => {
    ...
    function makeSection() {
        if (isAdmin) {
            ...
            section.id = 'tab-proof-review'; section.className = 'tab-content'; section.setAttribute('role', 'tabpanel');
            section.innerHTML = sectionMarkup(true); tabs.appendChild(btn); tabs.after(section);
            btn.addEventListener('click', () => {
                if (typeof window.switchAdminTab === 'function') window.switchAdminTab('proof-review', btn);
                    ...
                    section.classList.add('active'); btn.classList.add('active'); btn.setAttribute('aria-selected', 'true');
                }
                loadQueue();
            });
        } else {
            ...
            const btn = document.createElement('button'); btn.className = 'tab-btn'; btn.type = 'button';
            btn.dataset.tab = 'proof-review'; btn.setAttribute('role', 'tab'); btn.setAttribute('aria-selected', 'false'); btn.setAttribute('aria-controls', 'observerTab-proof-review'); btn.textContent = 'Proof verification';
            const section = document.createElement('section');
            section.id = 'observerTab-proof-review'; section.className = 'tab-content'; section.setAttribute('role', 'tabpanel'); section.innerHTML = sectionMarkup(false);
            tabs.appendChild(btn); tabs.after(section);
            btn.addEventListener('click', () => {
    ...
    function writeProofLoadingPage(tab) {
        ...
        try {
            tab.opener = null; tab.document.open();
    ...
    async function openProof(button, id) {
        ...
        try {
            ...
            if (!proofTab || proofTab.closed) { URL.revokeObjectURL(objectUrl); throw new Error('The proof window was blocked or closed. Allow pop-ups and try again.'); }
            proofTab.location.replace(objectUrl); window.setTimeout(() => URL.revokeObjectURL(objectUrl), 120000);
        } catch (error) {
            if (proofTab && !proofTab.closed) try { proofTab.document.body.innerHTML = `<div style="min-height:100vh;display:grid;place-items:center;background:#10151f;color:#e7ebf3;font-family:system-ui,sans-serif;padding:24px;box-sizing:border-box"><div style="max-width:420px;padding:18px;border:1px solid #47323a;border-radius:14px;background:#1b1820"><strong>Could not open proof</strong><p style="color:#c5c9d2">${esc(error.message)}</p></div></div>`; } catch (_) {}
            if (window.showToast) window.showToast(error.message, 'error'); else alert(error.message);
        } finally { button.disabled = false; button.textContent = originalText; }
    }

    function bindControls() {
        const root = scopeRoot(); if (!root || root.dataset.proofBound === 'true') return; root.dataset.proofBound = 'true';

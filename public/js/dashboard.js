...
async function markNotificationRead(id){const response=await fetch(`/api/student/workflow/notifications/${id}/read`,{method:'PUT'});const text=await response.text();let data={};try{data=JSON.parse(text);}catch(_){}if(!response.ok){showToast(data.error?.message||data.error||`Could not mark read (${response.status})`,'error');return;}await loadStudentNotifications();}
...
async function markAllNotificationsRead(){const button=document.querySelector('.notification-center-toolbar button');setButtonLoading(button,true,'Marking read');try{const response=await fetch('/api/student/workflow/notifications/read-all',{method:'PUT'});const text=await response.text();let data={};try{data=JSON.parse(text);}catch(_){}if(!response.ok)throw new Error(data.error?.message||data.error||`Could not mark notifications read (${response.status})`);await loadStudentNotifications();showToast('Notifications marked read.','success');}catch(error){showToast(error.message,'error');}finally{setButtonLoading(button,false);}}
